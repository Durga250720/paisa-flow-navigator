...
const RepaymentsContent = () => {
  ...
  const fetchRepayments = useCallback(async (page: number, currentFilters: Filters, searchStr: string) => {
    ...
    const requestBody = {
      ...
      searchRequest: searchStr || undefined, // Use undefined instead of null
      status: currentFilters.status !== 'ALL' ? currentFilters.status : undefined,
      startDate: currentFilters.startDate ? format(currentFilters.startDate, 'dd-MM-yyyy') : undefined,
      endDate: currentFilters.endDate ? format(currentFilters.endDate, 'dd-MM-yyyy') : undefined,
    };

    // Remove undefined values (cleaner than the null cleanup)
    const cleanRequestBody = Object.fromEntries(
      Object.entries(requestBody).filter(([_, value]) => value !== undefined)
    );

    try {
      const response = await axiosInstance.put(`${config.baseURL}repayment/filter`, cleanRequestBody);
      ...
      const errorMessage = err.response?.data?.message || 'Failed to fetch repayment data.';
